"""this is to run if you were in the middle of an experiment and accidentally closed the program like i did
uses same generated melodies list and just checks if each cf has had the fs done on it first"""
...
with open("generated_melodies.txt") as f:
    for line in f:
        ...
        expectedFilename = melodyToName(cfdict["melody"])
        expectedFilepath = Path("results/" + expectedFilename)

        if not expectedFilepath.is_file():
